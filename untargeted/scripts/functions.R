...
checkPeakCor <- function(mass, rtmin, rtmax, init_eic, 
  ...
  # plot(merged_eic$rt, scales::rescale(merged_eic$int.y), type="l")
  # lines(merged_eic$rt, scales::rescale(merged_eic$int.x))
...
findIsoAdduct <- function(file_peaks, xdata, grabSingleFileData,
                          checkPeakCor, pmppm, trapz, polarity, mzxml_path){
  file_peaks <- file_peaks[order(file_peaks$rtmax),]
  file_path <- paste0(mzxml_path, unique(file_peaks$filename))
  ...
  iso_matches_all <- lapply(peak_splits, function(i){
    ...
    iso_matches <- lapply(individual_peaks, function(peak_row_data){
      isos_to_check <- c(C13=1.003355, X2C13=2*1.003355, X3C13=3*1.003355,
                         X4C13=4*1.003355, X5C13=5*1.003355, N15=0.997035,
                         N15X1C13=1.003355+0.997035, N15X2C13=2*1.003355+0.997035,
                         N15X3C13=3*1.003355+0.997035, N15X4C13=4*1.003355+0.997035,
                         N15X5C13=5*1.003355+0.997035) + peak_row_data[["mz"]]
      masses_to_check <- c(isos_to_check)
      
      init_eic <- eic_many[mz%between%pmppm(peak_row_data["mz"], ppm = 5) & 
                             rt%between%c(peak_row_data["rtmin"], peak_row_data["rtmax"])]
      
      # print(ggplot(init_eic) + geom_line(aes(x=rt, y=int)))
  ...
  iso_matches_all <- lapply(peak_splits, function(i){
    ...
    iso_matches <- lapply(individual_peaks, function(peak_row_data){
      isos_to_check <- -c(C13=1.003355, X2C13=2*1.003355, X3C13=3*1.003355,
                         ...
                         N15X1C13=1.003355+0.997035, N15X2C13=2*1.003355+0.997035,
                         N15X3C13=3*1.003355+0.997035, N15X4C13=4*1.003355+0.997035,
                         N15X5C13=5*1.003355+0.997035, S34=1.995796, S33=0.999387,
                         O18=2.004244) + peak_row_data[["mz"]]
      if(polarity=="pos"){
      ...
      masses_to_check <- c(isos_to_check, adducts_to_check)
