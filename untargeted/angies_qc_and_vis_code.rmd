...
if(file.exists(paste0(output_folder,"potential_duplicated_good_MFs_akb.csv"))){
  similar.good.fts.to.keep <- read_csv(paste0(output_folder,"potential_duplicated_good_MFs_akb.csv"))
} else {
  similar.good.fts.to.keep <- cbind(good.fts, keep="y") %>%
    left_join(stan_annotations)
}
...
if(file.exists(paste0(output_folder,"Check_these_Features_akb.csv"))){
  curated.MFs <- read_csv(paste0(output_folder,"Check_these_Features_akb.csv")) %>%
    filter(keep_final == 'y')
} else {
